    ...
    async def run(
        ...
        *,
        ...
        while step_count < self.config.max_steps:
            ...
            try:
                llm_text, usage = await llm_call_fn(
                    messages=messages,
            ...
            # 3b. 解析工具调用
            tool_calls = parse_tool_call(llm_text)
            ...
            if tool_calls:
                ...
                if on_agent_step:
                    reasoning = _clean_thinking_text(_extract_reasoning_text(llm_text))
                    if reasoning.strip():
                        on_agent_step({
                ...
                # 构建工具结果消息
                tool_result_parts = ["【工具执行结果】\n"]
                for tr in step.tool_results:
                    status = "✓" if tr.success else "✗"
                    tool_result_parts.append(f"{status} {tr.tool_name} ({tr.duration_ms:.0f}ms):")
                    tool_result_parts.append(tr.content)
        ...
        if self.config.answer_evaluation and final_answer.strip():
            ...
            if _had_tools and _is_trivial and llm_call_fn and step_count < self.config.max_steps:
                ...
                try:
                    retry_text, _ = await llm_call_fn(
                        messages=messages,
                        max_tokens=0,
                    )
                    if retry_text.strip():
...
def _clean_thinking_text(text: str) -> str:
    ...
    问题：LLM tokenizer 可能把 <think> 切为 /<\`、\`think>\` 两个 token，
