...
def _build_tool_prompt(tools: Sequence[AgentTool]) -> str:
    """构建纯文本工具描述。用真实工具名做示例，避免小模型逐字复制模板。"""
    if not tools:
        return ""
    tool_texts = [t.to_prompt_text() for t in tools]
    tools_desc = "\n\n".join(tool_texts)

    # 用第一个真实工具构建具体示例，避免 "工具名/参数名" 占位符被小模型逐字复制
    first_tool = tools[0]
    first_param = ""
    props = first_tool.parameters.get("properties", {})
    if props:
        first_param = next(iter(props.keys()))
        first_param_desc = props[first_param].get("description", "")
        first_param_val = {
            "query": "电化学催化剂 d带中心",
            "paper_id": "1",
    ...
    return f"""【可用工具】
需要信息时，按以下格式调用工具（注意：name 和 arguments 必须用真实值，不要复制示例）：
...
规则：每次只调一个工具，拿到结果后再决定是否继续。信息够了就直接回答。"""
...
回答方式：直接给出结论，用简洁的段落组织。不要解释规则、不要复述问题、不要自我对话。"""
    ...
    if tools_str:
        ...
        example = f'{{"name":"{first_tool}","arguments":{{"query":"搜索关键词"}}}}' if first_tool == 'search_knowledge_base' else f'{{"name":"{first_tool}","arguments":{{}}}}'
        pattern = r'【可用工具】\n需要信息时，按以下格式调用工具[^\n]*\n\n<tool_call>\n\{.*?\}\n</tool_call>\n\n'
        simple_tools = re.sub(pattern, f'工具格式：\\n<tool_call>\\n{example}\\n</tool_call>\\n\\n', tools_str)
        simple_tools = simple_tools.replace("\n规则：每次只调一个工具，拿到结果后再决定是否继续。信息够了就直接回答。", "")
        simple_tools = simple_tools.replace(" (必填)", "").replace("默认5", "")
        simple_tools = re.sub(r'描述: .*?\n', '', simple_tools)
        simple_tools = re.sub(r'    - (\w+): .*?\n', r'  \1, ', simple_tools)
        base += f"\n\n{simple_tools}"
    return base
...
def build_agent_system_prompt(
    ...
    """
    tools = tools or []
    tools_str = _build_tool_prompt(tools) if tools else ""
