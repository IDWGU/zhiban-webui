...
def _truncate_result(text: str, max_chars: int = 2000) -> str:
    """截断过长结果，只保留头部。不做中部截断，避免模型看到残缺内容。"""
        ...
        **kwargs,
        ...
        try:
            ...
            for i, r in enumerate(results[:top_k], 1):
                content = r.get("doc", "")[:500]
                meta = r.get("meta", {})
                # vector_store uses doc_id / section_type / filename
                doc_id = meta.get("doc_id", meta.get("paper_id", "?"))
                section = meta.get("section_type", meta.get("section", ""))
                filename = meta.get("filename", "")
                score = 1.0 - r.get("dist", 1.0)
                relevance = f"相关度: {score:.2f}" if score else ""
                loc = f"Paper #{doc_id}"
                if section:
                    loc += f", {section}"
                if filename:
