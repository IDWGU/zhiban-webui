...
async def handle_translation_request(ws: WebSocket, msg: dict):
    ...
    # Each translation request gets a fresh cancel event.
    _translation_cancel = asyncio.Event()
    ...
    # Report errors if any sentences failed
    api_errors = result.get("errors", [])
    if api_errors:
        err_detail = api_errors[0] if len(api_errors) == 1 else f"{len(api_errors)} sentences failed: {api_errors[0]}"
        if result.get("tokens_sent", 0) == 0:
            # All sentences failed → error
            if _was_cancelled:
                return
            await ws.send_json({
                "type": "status", "level": "error",
                "code": "translation_error",
                "message": f"翻译失败: {err_detail}",
            })
            return
        else:
            ...
            await ws.send_json({
                ...
                "message": f"{len(api_errors)} 句翻译失败 — {err_detail}",
            })
