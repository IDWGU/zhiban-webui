...
class ThinkTagStreamSplitter:
    ...
    安全机制：
    - _NO_TAG_THRESHOLD (80 tokens): 无 tag 时自动退出检测模式
    - _MAX_THINKING_CHARS (3000 chars): thinking 模式超时强制退出，防止
      模型输出 <think> 但没有 </think> 导致所有后续内容被错误标为 thinking
    ...
    _MAX_TAG_LEN = 20
    _MAX_THINKING_CHARS = 3000  # 安全上限：thinking 模式最长 3000 字符
    ...
    def __init__(self):
        ...
        self._token_count = 0
        self._thinking_char_count = 0  # thinking 模式下的累计字符数
    ...
    def flush(self) -> list[tuple[bool, str]]:
        """清空剩余缓冲区。若 thinking 模式未正常关闭，强制以非 thinking 输出。"""
        results = []
        # 安全上限检查：若 thinking 字符已超限，强制退出 thinking 模式
        if self._in_think and self._thinking_char_count > self._MAX_THINKING_CHARS:
            self._in_think = False
            ...
            if not self._tag_detected:
                ...
                if m_close:
                    if m_close.start() > 0:
                        ...
                        self._thinking_char_count += len(think_text)
                        results.append((True, think_text))
                ...
                safe_end = self._safe_end(remaining)
                if safe_end > 0:
                    think_text = remaining[:safe_end]
                    self._thinking_char_count += len(think_text)

                    # 安全上限：thinking 模式超过 _MAX_THINKING_CHARS 字符
                    # 强制退出，防止 </think> 缺失导致所有内容被标为 thinking
                    if self._thinking_char_count > self._MAX_THINKING_CHARS:
                        self._in_think = False
                        self._thinking_char_count = 0
                        results.append((False, think_text))
                    else:
                        results.append((True, think_text))
                    self._cursor += safe_end
                break
...
def sync_call_llm(
    ...
    tools: OpenAI function calling 格式的工具定义列表（仅 API 模式生效）。
...
async def stream_call_llm(
    ...
    # cc-haha 风格: API 模式下 reasoning_content 已在协议层分离，
    # 不需要对 content 做 <think> 标签正则解析
    _is_local = is_local_mode(base_url)
    _think_splitter = ThinkTagStreamSplitter() if _is_local else None

    try:
        has_content_token = False
        reasoning_buffer = []
        tool_calls_structured: list[dict] | None = None
        _in_reasoning = False  # 当前是否在 reasoning 输出中
        ...
        async for chunk in provider.chat_stream(
            ...
            if chunk["type"] == "token":
                if _in_reasoning:
                    # reasoning_content 结束：关闭 <think> 标签
                    if on_token:
                        on_token("</think>", False)
                    _in_reasoning = False
                if on_token:
                    if _think_splitter is not None:
                        # 本地模式: 内容流中检测 <think> 标签
                        ...
                        # API 模式: content token 直接视为正文
                        has_content_token = True
                        total_tokens += 1
                        on_token(chunk["token"], False)
                else:
                    total_tokens += 1
                ...
                if _in_reasoning:
                    if on_token:
                        on_token("</think>", False)
                    _in_reasoning = False
                tool_calls_structured = chunk["tool_calls"]
            elif chunk["type"] == "done":
                if _in_reasoning and on_token:
                    on_token("</think>", False)
                    _in_reasoning = False
                total_tokens = chunk.get("total_tokens", total_tokens)
                finish_reason = chunk.get("finish_reason", finish_reason)

        # 清空 splitter 缓冲区（仅本地模式）
        if on_token and _think_splitter is not None:
        ...
        return {"input": 0, "output": 0, "elapsed": round(time.time() - t0, 2), "finish_reason": "error", "tool_calls": None}
