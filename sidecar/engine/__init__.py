...
class ZhibanV10:
    ...
    def __init__(self):
        ...
        # V12: Agent 注册中心 (cc-haha pattern)
        self._agent_registry = None  # AgentRegistry
    ...
    # ── V12: Agent 注册中心 + 执行器 (cc-haha pattern) ──

    def _get_agent_registry(self):
        """延迟初始化 AgentRegistry，注册内置 agent。"""
        if self._agent_registry is None:
            from ..agent.definitions import AgentRegistry
            from ..agent.builtin import register_all_builtin_agents
            self._agent_registry = AgentRegistry()
            register_all_builtin_agents(self._agent_registry)
            logger.info("AgentRegistry initialized with %d agents", len(self._agent_registry))
        return self._agent_registry

    def _resolve_agent_type(self, question_type: str, question: str) -> str:
        """根据问题类型选择最优 agent。

        cc-haha 对应: subagent_type resolution in AgentTool.call()
        """
        # summary 类型 → paper_summarizer
        if question_type == "summary":
            return "paper_summarizer"
        # 默认 → paper_assistant（全部工具可用）
    ...
    async def _agent_answer_llm_call(
        ...
        max_tokens: int = 0,
        **llm_params,
    ) -> tuple[str, dict, list[str], list[str], bool, list | None, str]:
        """Agent LLM 流式调用 — buffer 模式。

        只累积 token 不做任何前端推送。agent_loop 拿到完整文本后
        统一解析分类，再发送结构化事件到前端。
        彻底消除流式正则过滤的五层补丁。
        """
        system_prompt = ""
        ...
        def _buffer_token(token: str, is_thinking: bool = False):
            content_parts.append(token)
            token_count[0] += 1
            # 环路检测（仅检测，不阻断）
            if token_count[0] >= 40 and token_count[0] % 40 == 0:
                ...
                if detect_stream_loop(window) or detect_chinese_loop(window):
                    logger.warning("Stream loop suspected at token %d", token_count[0])
        ...
        try:
            ...
            # Pass finish_reason to agent loop for token overflow recovery
            stream_finish_reason = result.get("finish_reason")
            if stream_finish_reason:
                usage_info["finish_reason"] = stream_finish_reason

            if not content_parts and resp:
                content_parts = [resp]

            raw_text = "".join(content_parts)
            # Extract reasoning from <think>...</think> blocks (for API message history)
            import re as _re
            _think_match = _re.findall(r'<think>(.*?)</think>', raw_text, _re.DOTALL)
            reasoning_text = "\n".join(_think_match) if _think_match else ""

            logger.info(
                "Agent stream done: tokens=%d reasoning=%d raw=%d "
                "finish_reason=%s",
                token_count[0], len(reasoning_text), len(raw_text),
                result.get("finish_reason", "N/A"),
            )

            tool_calls = result.get("tool_calls")
            # Return raw_text (with <think> tags) so the frontend's </think>
            # parser can split thinking from answer. reasoning_text extracted
            # separately for API message history.
            return raw_text, usage_info, tool_calls, reasoning_text
        except asyncio.CancelledError:
            raise
        except Exception as e:
    ...
    async def _run_agent(
        ...
        """使用 Agent 循环处理用户问题 (V12 — cc-haha pattern)。

        通过 AgentRegistry 获取 AgentDefinition，
        通过 AgentExecutor 过滤工具、构建 prompt，
        AgentLoop 执行实际的 while 循环。
        """
        # V12: 从 AgentRegistry 获取定义
        question_type = _detect_question_type(question)
        agent_type = self._resolve_agent_type(question_type, question)
        registry = self._get_agent_registry()
        definition = registry.get(agent_type)
        if definition is None:
            definition = registry.get("paper_assistant")
        if definition is None:
            raise WorkflowError("agent", "No agent definition available")

        # V12: 按定义过滤工具
        all_tools = self._get_agent_tools()
        filtered_tools = [t for t in all_tools if definition.allows_tool(t.name)]
        logger.info("Agent '%s' running with %d/%d tools",
                     agent_type, len(filtered_tools), len(all_tools))

        agent_config = self._get_agent_config()
        agent_config.answer_max_tokens = max_tokens
        agent_config.max_steps = definition.max_turns
        ...
        # V12: 从 AgentDefinition 构建 system prompt
        from ..agent.prompts import build_agent_system_prompt_v2
        from ..agent.memory import load_agent_memory, build_memory_prompt
        ...
        memory_text = ""
        if definition.memory_scope:
            try:
                memory_text = load_agent_memory(agent_type, scope=definition.memory_scope)
            except Exception:
                pass

        system_prompt = build_agent_system_prompt_v2(
            definition=definition,
            tools=filtered_tools,
            knowledge_brief=knowledge_brief,
            memory_text=memory_text,
            native_tools=True,  # cc-haha style: API 原生 tools 参数，结构化 tool_calls
        )
        ...
        # Convert tools to OpenAI function calling format for API mode
        _api_tools = [t.to_openai_schema() for t in filtered_tools]
        _params_with_tools = {**llm_params, "tools": _api_tools}

        # Non-streaming eval function to prevent eval reasoning from leaking to frontend
        async def _eval_llm_fn(messages, max_tokens=0, **params):
            system_prompt_eval = ""
            user_msgs = list(messages)
            if messages and messages[0].get("role") == "system":
                system_prompt_eval = messages[0]["content"]
                user_msgs = messages[1:]
            params.pop("tools", None)
            text, usage = await asyncio.to_thread(
                sync_call_llm,
                system_prompt_eval, user_msgs,
                max_tokens=max_tokens,
        ...
        async def _agent_llm_fn(messages, max_tokens, **params):
            ...
            text, usage, tool_calls, reasoning_text = \
                await self._agent_answer_llm_call(
                    messages, stream_max_tokens, **params)

            # 循环检测 → 升温重试（温度递增引入随机性打破循环）
            # Stream loop detection disabled: always False in current LLM streaming.
            loop_detected = False
            ...
            if loop_detected and retries < 3:
                ...
                retry_text, retry_usage, _loop_flag = await self._agent_llm_call(
                    retry_msgs, max_tokens, **retry_params
                )
                tool_calls = None  # retry uses sync path, tool_calls not extracted here
                reasoning_text = ""  # retry path: no reasoning available
                if retry_text.strip():
                    text = retry_text
                    usage = retry_usage
                    if on_token:
            ...
            return text, usage, tool_calls, reasoning_text
        ...
        try:
            result: AgentResult = await loop.run(
                ...
                system_prompt_override=system_prompt,  # V12: AgentDefinition 的 prompt
                active_slice=active_slice,
                knowledge_brief=knowledge_brief,
                llm_call_fn=_agent_llm_fn,
                llm_params=_params_with_tools,
                cancel_event=self._cancel_event,
                on_agent_step=on_agent_step,
                eval_llm_call_fn=_eval_llm_fn,
                native_tools=True,  # cc-haha style: API 原生 tools 参数，结构化 tool_calls
            )
        except asyncio.CancelledError:
            raise
