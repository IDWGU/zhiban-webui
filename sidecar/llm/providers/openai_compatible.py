...
class OpenAICompatibleProvider:
    ...
    async def chat_stream(
        ...
        *,
        ...
        # Accumulate native tool_calls from streaming deltas
                    ...
                    for tc_d in tc_deltas:
                        ...
                        if tc_d.function:
                            if tc_d.function.name:
                                tc["name"] += tc_d.function.name
                            if tc_d.function.arguments:
                                tc["arguments"] += tc_d.function.arguments
        ...
        # Yield accumulated native tool_calls as structured data only (cc-haha style).
        # No XML tokens in content stream — they pollute llm_text and break
        # multi-round message history for DeepSeek's API.
        if accumulated_tool_calls:
            ...
            if structured_calls:
                yield {"type": "tool_calls", "tool_calls": structured_calls}
    ...
    def chat_sync(
        ...
        *,
        ...
        api_tools = None
        if tools:
            api_tools = tools if isinstance(tools, list) else None
        ...
        try:
            ...
            # Extract native tool_calls — keep as structured data, don't inject XML
            # into text content (breaks DeepSeek multi-round message format)
            native_tool_calls = getattr(resp.choices[0].message, 'tool_calls', None)
