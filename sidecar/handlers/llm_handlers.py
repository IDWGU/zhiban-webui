...
async def handle_user_query(ws: WebSocket, msg: dict):
    ...
    # Auto-detect: if base_url is __local__ (default) AND local engine is loaded, use
    # it. If user sent an explicit API URL (e.g. deepseek), respect it — never override
    # an explicit API choice with local mode just because a local engine is present.
    local_check = get_local_engine()
    if local_check and local_check.is_loaded and is_local_mode(llm_base_url):
        # __local__ + engine loaded → use local model path
